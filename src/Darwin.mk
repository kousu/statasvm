...
$(NAME).so: $(OBJECTS) # hack: we are trying to build .c -> .o -> .so -> .dylib on Darwin, but .so doesn't gets told to depend on the .o files (instead the dylib does) so building the .so fails. This patches over that with toothpaste and popsicle sticks. Hmmmmmm. I'll have to think about this.
