...
// A first guest: the shape is the same for white and black respondents, but white respondents have a wider range.
twoway (scatter wage hours), by(race)
pause "Type q to continue."

// We will now ask one-class SVM to detect the shape of that less varied region, to give us a sense of the black labour market in 1988.
...
predict S
twoway (scatter wage hours if !S) (scatter wage hours if S)
pause "Type q to continue."
...
twoway (scatter wage hours if !S, jitter(5)) (scatter wage hours if S, jitter(5))
pause "Type q to continue."
